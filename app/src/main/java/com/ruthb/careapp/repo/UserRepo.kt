...
//        mDatabaseReference = mDatabase!!.reference!!.child("Users")
    ...
    fun loginSocial(tokenCredential: AuthCredential) {
        ...
        try {
            mAuth!!.signInWithCredential(tokenCredential).addOnCompleteListener {
                if (it.isSuccessful) {
    ...
    fun sendUserInfo(userInfo: UserInfo) {
        try {
            mDatabaseReference = mDatabase!!.reference!!.child("Users")
            mDatabaseReference?.child(mAuth?.currentUser!!.uid)?.setValue(userInfo)
            println("nome recebido: ${userInfo.username}")
